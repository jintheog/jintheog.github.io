...
JavaScript를 경험한 개발자라면 이미 프로그래밍의 기본 개념을 알고 있을 것입니다. 위에서 살펴본 것처럼 Java는 JavaScript와 이름은 비슷하지만 완전히 다른 언어입니다.
...
JavaScript는 스크립트를 작성하면 바로 실행되지만, Java는 소스 코드를 컴파일하여 바이트코드로 변환한 후 JVM에서 실행됩니다.
